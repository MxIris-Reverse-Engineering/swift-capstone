...
| 2026-09-21 | 记录：v6 重排了 `cs_arch` 的数值 | 生成架构表时发现 v5 的 `CS_ARCH_SYSZ = 6` 在 v6 变为 `CS_ARCH_SYSTEMZ = 2`，MIPS / X86 / PPC / SPARC 全部后移一位。若按原计划手工改名而不改数值，`Architecture.mips` 会把 2 传给 `cs_open`，即 v6 的 SystemZ —— 产生错误的反汇编结果且不报任何错。这是「从头文件解析而非手工维护」这一决定的直接实证，已写入 `Architecture.swift` 的注释。运行时验证：23 个 case 数值全部与 v6 头文件一致。 |
| 2026-09-21 | 修复 capstone 侧的 guard 遗漏（由本工作暴露） | swift-capstone 默认 trait 为空集触发了「关闭 X86」的构建路径，暴露出 `arch/X86/X86InstPrinterCommon.c` 缺 `CAPSTONE_HAS_X86` guard：`UpdateArchitectureGuards.sh` 用裸 `CAPSTONE_HAS_` 前缀匹配，把与架构无关的 `CAPSTONE_HAS_OSXKERNEL` 误判成架构 guard 而跳过该文件。此前验证 guard 的三种组合均启用了 X86，该缺陷在启用时不可见。已在 capstone 仓库修复（commit `81d9ca83`），并为脚本加入 `--check` 模式作为回归检查。 |
